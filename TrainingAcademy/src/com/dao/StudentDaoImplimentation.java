...
public class StudentDaoImplimentation implements StudentDao
{
	...
	public void topBatchAndTrainerr()
	{
			...
			PreparedStatement preparedStatement=null;
			ResultSet resultSet=null;
			String 	query="select b.batchid,b.batch_name, t.Trainer_name, avg(r.percent) as average\r\n"
					+ "from student s join trainer t join batches b join result r on s.batchid=b.batchid \r\n"
					+ "and s.studId=r.studid and b.batchid=t.Batch_id group by b.batchid,t.trainer_name order by average desc limit 1";
			try
			{
				connection = ConnectionFactory.getConnection();
				preparedStatement=connection.prepareStatement(query);
